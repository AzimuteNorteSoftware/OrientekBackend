...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
 ...
 rua.nomeRua
  ...
  WHERE 
    ...
    bairro.idCidade = cidade.idCidade AND
    cidade.idEstado = estado.idEstado";
            $queryParticipacao = "SELECT pagoParticipacao FROM sistema_oriente.participacao WHERE idCorredor={$UId} AND idEvento={$_GET['id']};";
            ...
            if ($endereco->num_rows && $res->num_rows) {
                ...
                if ($inscrito->num_rows > 0) $r['inscrito'] = true;
                echo json_encode($r);
            }
        } else {
            echo $db->error();
    ...
    if (isset($data['evento'])) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'],   $newfilename)) {
            $sql = $db->query('UPDATE participacao SET comprovante = "' . $newfilename . '" WHERE idCorredor= ' . $UId . ' and idEvento=' . $_POST['evento'] . ';');
            echo $db->error();
            if ($sql) {
                $r = true;
            } else {
